...
function library:New(name)      
	...
	Uwu.Name = "Gifted likes Femboys"      
	Uwu.ResetOnSpawn = false      
	Uwu.ZIndexBehavior = "Global"      
	Uwu.DisplayOrder = 420133769      
	...
	cursor.ZIndex = 1000      
	cursor.ImageColor3 = COL3RGB(255,3,13)      
	...
	Menu.Image = "rbxassetid://8062345037"      
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key)      
		if key.KeyCode == Enum.KeyCode.Insert then      
			Uwu.Enabled = not Uwu.Enabled      
			library.uiopen = Uwu.Enabled      
		end      
	end)      
	...
	TextLabel.Text = name  
	TextLabel.TextColor3 = COL3RGB(255, 0, 0)  
	TextLabel.TextSize = 13.000  
	TextLabel.TextXAlignment = Enum.TextXAlignment.Left  

	TabButtons.Name = "TabButtons"  
	TabButtons.Parent = Menu  
	TabButtons.BackgroundColor3 = COL3RGB(255, 0, 0)  
	...
	Tabs.Name = "Tabs"  
	Tabs.Parent = Menu  
	Tabs.BackgroundColor3 = COL3RGB(255, 0, 0)  
	...
	function menu:Tab(text)  
		...
		TabGui.Name = "TabGui"  
		TabGui.Parent = Tabs  
		TabGui.BackgroundColor3 = COL3RGB(255, 0, 0)  
		...
		Left.Name = "Left"  
		Left.Parent = TabGui  
		Left.BackgroundColor3 = COL3RGB(255, 0, 0)  
		...
		Right.Name = "Right"  
		Right.Parent = TabGui  
		Right.BackgroundColor3 = COL3RGB(255, 0, 0)  
...
for _,lua in pairs(listfiles("uwu.lua")) do  
	local luaname = GSUB(lua, "uwu.lua\\", "")  
	INSERT(allluas, luaname)  
end  

RunService.RenderStepped:Wait()      

local gui = library:New("Avidity")      
...
luascripts:Element("Button", "load", {}, function()  
	loadstring(readfile("uwu.lua\\"..values.luas["lua scripts"].lua.Scroll))()  
end)  
...
luascripts:Element("Button", "Trail", nil, function()
...
local function AddLegit(Tab)      
	...
	Tab:Element("Slider", "field of view", {min = 30, max = 660, default = 120})      
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "dogshit", "180", "funny", "wind", "wind2", "wind3", "random", "God", "fucker", "cool", "cool2", "back"}})  
...
antiaim:Element("Dropdown", "body roll", {options = {"off", "on", "-4", "-15", "-30", "-60", "-180", "-360", "-374", "-262"}})
antiaim:Element("Slider", "body roll offset", {min = -180, max = 180, default = 4})  
antiaim:Element("Slider", "spin speed", {min = 1, max = 180, default = 4})  
antiaim:Element("Slider", "High poss", {min = -2, max = 20, default = 2})

local others = rage:Sector("others", "Right")  
others:Element("Toggle", "remove head")  
others:Element("Toggle", "no animations")  
others:Element("Dropdown", "leg movement", {options = {"off", "No Jump", "Slide"}})  
...
self:Element("Slider", "distance", {min = 6, max = 24, default = 12}, function(tbl)      
	if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then      
...
self:Element("Slider", "fov changer", {min = 0, max = 150, default = 80}, function(value)      
	RunService.RenderStepped:Wait()      
...
self:Element("Slider", "viewmodel x", {min = -25, max = 25}, function(val)      
...
self:Element("Slider", "viewmodel y", {min = -25, max = 25}, function(val)      
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)      
end)      
self:Element("Slider", "viewmodel z", {min = -25, max = 25}, function(val)      
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)      
end)      
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "1nn", "oni-chan", "Bonk", "Welcome", "Semi", "osu", "Tf2", "Tf2 pan", "M55solix", "Slap","Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "funny", "dababy", "quek"}})  
world:Element("Slider", "sound volume", {min = 1, max = 9, default = 3})      
...
movement:Element("Slider", "speed", {min = 0, max = 250, default = 40})      
...
local Avidityxyz = Instance.new("ScreenGui")
local Main = Instance.new("TextButton")
local Frame = Instance.new("Frame")
    

Avidityxyz.Name = "Avidity.xyz"
Avidityxyz.Parent = game.CoreGui
Avidityxyz.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

    
Main.Size = UDim2.new(0,200,0,40)
Main.Transparency = 1
Main.Parent = Avidityxyz
...
CreateHitElement("Loading Avidity.xyz..",Color3.new(1,1,0))
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then      
			...
			if values.rage.angles["body roll"].Dropdown == "on" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "on" and RAD(values.rage.angles["body roll offset"].Slider) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end  
            if values.rage.angles["body roll"].Dropdown == "-4" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-4" and RAD(RANDOM(-4, 4)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-15" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-15" and RAD(RANDOM(-15, 15)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-30" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-30" and RAD(RANDOM(-30, 30)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-60" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-60" and RAD(RANDOM(-60, 60)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-180" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-180" and RAD(RANDOM(-180, 180)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-360" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-360" and RAD(RANDOM(-360, 360)) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider 
			else  
				LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["High poss"].Slider  
			end
			if values.rage.angles["body roll"].Dropdown == "-374" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-374" and RAD(213) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  

      if values.rage.angles["body roll"].Dropdown == "-262" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "-262" and RAD(90) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and 0 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "dogshit" and -5/93 -13 or values.rage.angles["pitch"].Dropdown == "funny" and 20 or values.rage.angles["pitch"].Dropdown == "wind" and math.huge -5/0 -1 or values.rage.angles["pitch"].Dropdown == "wind2" and 23 or values.rage.angles["pitch"].Dropdown == "cool2" and -13 or values.rage.angles["pitch"].Dropdown == "cool" and 12 or values.rage.angles["pitch"].Dropdown == "back" and -92 or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-100, 100)/10 or 2.5  
...
mt.__namecall = function(self, ...)      
	...
	if method == "LoadAnimation" and self.Name == "Humanoid" then  
		if values.rage.others["leg movement"].Dropdown == "No Jump" then 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Всем привет! Меня зовут Влад А4Глент кобяков и представляетеЭто мой первый хит, да! Ха-хаПодписчики, я вас люблю! (Погнали!)Это Ламба (Врум-врум)А это ГеликВлад-Влад это — бумага, а бумага — это деньги (Ви-и!)", false, "Innocent", false, true)  
	end      
end)      
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/wind777777/wm/main/wm"))()
...
game:GetService("TweenService"):Create(Watermark, TweenInfo.new(.25), {Position = Position}):Play()
end
...
ChatScript.createNewMessage("Avidity","Build: Private",MainUIColor,Color3.new(1,1,1),0.01,nil)
